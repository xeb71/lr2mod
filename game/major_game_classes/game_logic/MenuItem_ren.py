...
def build_menu_item_list(element_list, draw_hearts_for_people = True, draw_person_previews = True, show_location = False, person_preview_args = None,
    ...
    for item in column_elements:
        ...
        if isinstance(item, Person): #It's a person. Format it for a person list.
            ...
            if item.is_queen:
                info.append("{image=full_crown_token_small}")
            ...
            if any((draw_insta, draw_dikdok, draw_onlyfans)):
                ...
                if draw_insta:
                    req_type = item.event_triggers_dict.get("insta_special_request_type", None)
                    if req_type:
                        if item.event_triggers_dict.get("insta_special_request_responded", False):
                            info.append("\n{color=#00ff00}{size=14}" + req_type + "{/size}{/color}")
                        elif item.event_triggers_dict.get("insta_special_request_pending", False):
                            if item.event_triggers_dict.get("insta_special_request_feasible", None) is False:
                                info.append("\n{color=#ff0000}{size=14}" + req_type + "{/size}{/color}")
                            else:
                                info.append("\n{color=#ffffff}{size=14}" + req_type + "{/size}{/color}")
            else:
                if item.is_clone:
                ...
                if item.serum_effects:
                    ...
                    elif item.active_serum_count == item.serum_tolerance:
                ...
                if item.bc_status_known and item.is_highly_fertile and perk_system.has_ability_perk("Ovulation Cycle Perception"):
                    info.append("{image=fertile_token_small}")
            ...
            if getattr(item, 'has_phone_photo', False):
                mi.portrait_displayable = item.build_person_portrait()
        ...
        if isinstance(item, Action):
            ...
            if item.menu_tooltip:
                if isinstance(item.menu_tooltip, str):
                    tooltip_str = find_and_replace_tooltip_property(item, mi.extra_args)
                else:
                    tooltip_str = item.menu_tooltip(mi.extra_args)
                mi.the_tooltip = Text(tooltip_str, substitute=True).get_all_text()

        if isinstance(item, str): #It's just text. Display the text and return the text.
            mi.title = item
            mi.return_value = item
        ...
        if mi.display:
            if isinstance(item, Person) and isinstance(item.title, str) and isinstance(mi.the_tooltip, str):
...
class MenuItem():
    def __init__(self, title = "", return_value = None, the_tooltip = None, extra_args = None, display = True, is_sensitive = True, display_key = None, display_func = None, person_preview_args = None):
        ...
        self.portrait_displayable = None
