...
class StripteasePosition():
    def __init__(self, name,
        ...
        self.leads_to = [] #Other positions should add a tuple of (position, command description) to this list.

    def __hash__(self):
        return hash(self.name)

    def __eq__(self, other: StripteasePosition) -> bool:
        if not isinstance(other, StripteasePosition):
    ...
    def call_pose(self, the_person, direction):
        the_person.draw_person(position = (self.position_away_pose if direction == "away" else self.position_towards_pose))

    def has_acceptable_transition(self, the_person):
        return any(x for x in self.leads_to if x[0].slut_requirement < the_person.effective_sluttiness())
