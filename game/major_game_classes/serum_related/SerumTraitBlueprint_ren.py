...
import renpy
from game.major_game_classes.serum_related.SerumTrait_ren import SerumTrait, mc
"""renpy
...
class SerumTraitBlueprint(SerumTrait): #A Serum Trait Blueprint is a trait that describes a general class of similar actions - like changing hair colour.
    ...
    def __init__(self, *args, unlock_label = None, **kwargs):
    ...
    def unlock_trait(self, pay_clarity = True):
        ...
        if self.unlock_label: #If the unlock_trait removes itself from the blueprinted_traits list we don't name it - it was cancelled because some input was invalid.
