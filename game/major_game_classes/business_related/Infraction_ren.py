...
from game.business_policies.organisation_policies_ren import strict_enforcement, draconian_enforcement
"""renpy
...
class Infraction():
    ...
    @staticmethod
    def bureaucratic_mistake_factory(name = "Bureaucratic Mistake", desc = None, severity = 1, days_valid = 3) -> "Infraction":
        if desc is None:
    ...
    @staticmethod
    def underperformance_factory(name = "Under Performance", desc = None, severity = 1, days_valid = 7) -> "Infraction":
        if desc is None:
            desc = "Work performance lower than expected of the employee."
        return Infraction(name, desc, severity, days_valid)

    @staticmethod
    def careless_accident_factory(name = "Careless Accident", desc = None, severity = 2, days_valid = 7) -> "Infraction":
        if desc is None:
            desc = "Damage to company equipment or waste of company supplies due to a careless mistake."
        return Infraction(name, desc, severity, days_valid)

    @staticmethod
    def office_disturbance_factory(name = "Workplace Disturbance", desc = None, severity = 2, days_valid = 7) -> "Infraction":
        if desc is None:
            desc = "Actions that have upset the normal peace and quiet of the office."
        return Infraction(name, desc, severity, days_valid)

    @staticmethod
    def out_of_uniform_factory(name = "Out of Uniform", desc = None, severity = 3, days_valid = 7) -> "Infraction":
        if desc is None:
            desc = "Failure to wear a company mandated uniform."
        return Infraction(name, desc, severity, days_valid)

    @staticmethod
    def disobedience_factory(name = "Disobedience", desc = None, severity = 3, days_valid = 7) -> "Infraction":
    ...
    @staticmethod
    def inappropriate_behaviour_factory(name = "Inappropriate Behaviour", desc = None, severity = 3, days_valid = 7) -> "Infraction":
        if desc is None:
            desc = "Actions inappropriate for a workplace setting. Strange how this never applies to the management..."
        return Infraction(name, desc, severity, days_valid)

    def __init__(self, name: str, desc: str, severity: int, days_valid: int, days_existed = 0):
        ...
        self.severity = severity #An int from 1 (least severe) to 5 (most severe). Punishments are gated by the severity of the infraction
