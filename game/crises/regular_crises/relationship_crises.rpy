...
label so_relationship_improve_label():
    ...
    if the_person is None or the_person.is_stranger:
    ...
    $ the_person.set_event_day("relationship_changed")
    return
...
label so_relationship_worsen_label():
    ...
    if the_person.is_affair:
        ...
        the_person "Hey [the_person.mc_title], I've got some news. Me and my [the_person.so_title], [the_person.SO_name], had a fight. We aren't together any more."
        ...
        $ the_person.change_happiness(-20)
        "You get a notification on your phone."
        "It looks like [the_person.title] has left her [the_person.so_title] and is single now."
...
label so_relationship_quarrel_label(the_person):
    if renpy.random.randint(0, 100) >= breakup_chance_relationship[the_person.relationship]:
        if renpy.random.randint(0,3) == 1: # 25% change to complain, otherwise go to talk
            the_person "Hey [the_person.mc_title], it's good to see you. Me and my [the_person.so_title], [the_person.SO_name], had a fight."
    ...
    $ the_person.change_happiness(-20)
    the_person "Hey [the_person.mc_title], it's good to see you. Me and my [the_person.so_title], [the_person.SO_name], had a fight and we decided to split up."
...
label affair_dick_pick_label():
    $ the_person = get_phone_sexting_person()
    ...
    "[the_person.possessive_title!c] is lying face up in her bed, one hand cradling a breast while the other fingers her wet pussy."
    menu:
        ...
        "Tell her you're busy":
            ...
            if the_person.is_submissive:
                $ the_person.change_stats(obedience = 2, slut = 2 + the_person.opinion.cheating_on_men, max_slut = 70)
                ...
                $ the_person.change_stats(obedience = 2, love = -1, slut = 1 + the_person.opinion.cheating_on_men, max_slut = 40)
    ...
    $ the_person.set_event_day("phone_sexting")
    $ mc.end_text_convo()
    $ clear_scene()
    return
...
label girlfriend_nudes_label():
    ...
    if the_person.effective_sluttiness() < 20:
        ...
        $ the_person.draw_person(position = "missionary")
        "[the_person.possessive_title!c] arches her back as the vibrator touches her clit."
        ...
        $ the_person.update_outfit_taboos()
        $ the_person.apply_outfit(the_person.planned_outfit)
    #TODO: A blowjob/deepthroat training video, or an anal stretching video she sends you to show she's "getting ready."
    $ mc.end_text_convo()
    $ clear_scene()
    $ the_person.set_event_day("phone_sexting")
    return
...
label friends_help_friends_be_sluts_label():
    ...
    if person_one.effective_sluttiness() < 30: #If our slutty person isn't very slutty in the first place.
        ...
        $ scene_manager.add_actor(person_one, person_one.current_planned_outfit, position = "walking_away")
        $ scene_manager.add_actor(person_two, person_two.current_planned_outfit, position = "walking_away", display_transform = character_center_flipped)
        menu:
            "Stop to listen":
                ...
                $ play_spank_sound()
                "[person_one.possessive_title!c] slaps [person_two.title]'s butt."
        ...
        menu:
            "Investigate":
                if person_two.effective_sluttiness() < 30: #But the other girl is low sluttiness.
                    ...
                    $ person_two.change_stats(obedience = 2, slut = 2)
                    $ mc.change_locked_clarity(5)
                    ...
                    $ person_two.change_stats(love = 1, slut = 1 + person_two.opinion.public_sex)
                    $ mc.change_locked_clarity(10)
        ...
        $ scene_manager.add_actor(person_one, person_one.current_planned_outfit, position = "stand2")
        $ scene_manager.add_actor(person_two, person_two.current_planned_outfit, position = "stand3", display_transform = character_center_flipped)
        menu:
            "Investigate":
                ...
                if person_two.effective_sluttiness() < 20:
                    ...
                    # When you walk in the sluttier girl makes some passes at you that her friend apologises for, but that you re-enforce.
                    ...
                    "[person_one.possessive_title!c] notices you at the door."
                    $ scene_manager.update_actor(person_two, emotion = "sad")
                    ...
                    menu:
                        "[person_one.fname] has nicer tits": #She's already slutty, but gets a love boost
                            ...
                            $ person_one.change_stats(happiness = 5, love = 1 + person_one.opinion.showing_her_tits)
                            $ scene_manager.update_actor(person_two, emotion = "sad")
                        ...
                        "[person_two.fname] has nicer tits": # She gets a sluttiness boost along with a small love boost.
                            ...
                            $ person_two.change_stats(slut = 2 + person_one.opinion.showing_her_tits, love = 1 + person_one.opinion.showing_her_tits)
                            $ scene_manager.update_actor(person_two, emotion = "happy")
                        ...
                        "I'm going to need a closer look" if not person_one.tits_visible or not person_two.tits_visible: #Requires high obedience, sluttiness, or a uniform policy for the less slutty girl.
                            ...
                            if not person_two.tits_visible:
                                ...
                                if person_two.opinion.showing_her_tits > 0:
                            ...
                            menu:
                                "[person_one.fname] has nicer tits": #She's already slutty, but gets a love boost
                                    ...
                                    $ person_one.change_stats(happiness = 5, love = 1 + person_one.opinion.showing_her_tits)
                                    $ person_two.change_slut(1 + person_two.opinion.showing_her_tits)
                                ...
                                "[person_two.fname] has nicer tits": # She gets a sluttiness boost along with a small love boost.
                                    ...
                                    $ person_two.change_stats(slut = 1 + person_one.opinion.showing_her_tits, love = 1 + person_one.opinion.showing_her_tits)
                                    $ scene_manager.update_actor(person_one, emotion = "happy")
                    ...
                    if person_two.effective_sluttiness("sucking_cock") < 50: #She's embarrassed, but wants to do it
                        ...
                        $ scene_manager.update_actor(person_two, emotion = "default")
                        $ mc.change_locked_clarity(20)
                        if person_two.opinion.giving_blowjobs < 0:
                            ...
                            "[person_two.possessive_title!c] nods, blushing intensely."
                    ...
                    menu:
                        "Let [person_two.fname] give you a blowjob":
                            ...
                            $ scene_manager.update_actor(person_two, emotion = "happy")
                            if person_one.has_relation_with_mc:
                            ...
                            "When you are finished, she stands up."
                            $ person_two.draw_person()
                            $ the_report = _return
                            ...
                            $ scene_manager.update_actor(person_two, position= "walking_away")
                            "[person_two.possessive_title!c] takes a moment to get herself tidied up, then steps out of your office."
...
label work_relationship_change_label():
    $ the_relationship = get_random_from_list(
        [x for x in town_relationships.get_business_relationships(["Acquaintance"])
...
label friend_sends_text():
    ...
    menu:
        ...
        "Ignore her":
            ...
            if ((the_person.opinion.showing_her_tits > 0 or the_person.opinion.showing_her_ass > 0) and the_person.effective_sluttiness() >= 30) or the_person.is_affair:
