...
def normalize_hex(hex_value) -> str:
    """
    ...
    * The value is normalized to lower-case.
    ...
    try:
        ...
        msg = f"'{hex_value}' is not a valid hexadecimal color value."
...
def name_to_hex(name, spec='css3'):
    ...
    if spec not in SUPPORTED_SPECIFICATIONS:
        msg = "'{}' is not a supported specification for color name lookups; supported specifications are: {}.".format(
            spec,
    ...
    try:
        ...
        msg = f"'{name}' is not defined as a named color in {spec}."
        raise ValueError(msg) from ex
    return hex_value
...
def hex_to_name(hex_value, spec='css3'):
    ...
    try:
        ...
        msg = f"'{hex_value}' has no defined color name in {spec}."
        raise ValueError(msg) from ex
    return name
...
def _percent_to_integer(percent):
    ...
    e = num - floor(num)
    return int(floor(num)) if e < 0.5 else int(ceil(num))
...
def closest_colour(requested_colour):
    ...
    return min_colours[builtins.min(min_colours.keys())]
