...
init -1 python:
    def fetish_lily_stream_in_room_requirement(person):
...
label fetish_mom_kitchen_label(the_person):
    ...
    menu:
        "Fuck her ass" if the_person.has_anal_fetish:
            ...
            call fuck_person(the_person, start_position = anal_standing, start_object = make_table(), skip_intro = True, self_strip = False, skip_condom = True) from _call_sex_description_SBR40
            $ the_report = _return
        ...
        "Cover her in cum" if the_person.has_cum_fetish:
            mc.name "Hey [the_person.title]... I have something better than vegetables for you."
            "[the_person.possessive_title!c] stiffens for a second, then slowly looks over her shoulder at you."
            the_person "...What are you doing?"
            mc.name "Just keep working on dinner. Don't stop."
            "[the_person.possessive_title!c] hesitates, then turns back to the counter. Her hands resume chopping but her breathing has already changed."
            "You step close behind [the_person.possessive_title] and pull yourself out."
            $ mc.change_locked_clarity(30)
            $ mc.change_arousal(15)
            the_person "W-we can't... your sister could walk in any second..."
            mc.name "Then you'd better be quiet."
            $ mc.change_locked_clarity(30)
            $ mc.change_arousal(20)
            "[the_person.possessive_title!c] reaches back with one hand and starts to stroke you while the other keeps up the pretence of cooking."
            the_person "Mmmm... I can feel how close you are already..."
            $ mc.change_locked_clarity(30)
            $ mc.change_arousal(20)
            mc.name "Stay still. Here it comes."
            if the_person.effective_sluttiness() >= 60:
                "[the_person.title] arches her back, pushing her ass toward you eagerly."
                the_person "Yes... cover me in it!"
                $ ClimaxController.manual_clarity_release(climax_type = "ass", person = the_person)
                $ the_person.cum_on_ass()
                "You groan under your breath and let yourself go, spilling your load across [the_person.possessive_title]'s ass and lower back."
                the_person "Ahhhh... god... I love that feeling."
                "[the_person.title] runs a finger along her back and brings it to her lips, tasting it."
                $ the_person.change_obedience(2, 150)
            else:
                "[the_person.title] braces herself against the counter."
                $ ClimaxController.manual_clarity_release(climax_type = "ass", person = the_person)
                $ the_person.cum_on_ass()
                "You let out a quiet moan as you cum, covering [the_person.possessive_title]'s ass and lower back with your seed."
                the_person "Oh my... that's... a lot."
                "She glances down, taking in the mess you made of her."
                the_person "That's going to be hard to hide from your sister..."
                mc.name "You'll manage."
        ...
        "Fuck her loudly" if the_person.has_exhibition_fetish:
            ...
            call fuck_person(the_person, start_position = standing_doggy, start_object = make_table(), skip_intro = True, self_strip = False, skip_condom = True) from _call_sex_mom_kitchen_loud_sex_01
            $ the_report = _return
...
label fetish_lily_stream_in_room_label(the_person): # NOTE: This scene is currently disabled. As Lily progresses, give her a scene where you can act out her fetishes on video stream
    ...
    call fuck_person(the_person, start_position = doggy_anal_dildo_dp, start_object = make_bed(), skip_intro = True, skip_condom = True) from _call_lily_fetish_stream_anal_01
