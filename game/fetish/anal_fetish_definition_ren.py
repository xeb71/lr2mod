...
def get_needy_anal_fetish_employee(): #If it's been a while
    return get_random_from_list([
        x for x in mc.business.employees_availabe
        if x.has_anal_fetish
        and x.days_since_event("LastAnalFetish") > 10
        and not x.event_triggers_dict.get("anal_fetish_crisis_disabled", False)
...
def add_anal_fetish(person: Person):
...
def debug_set_stats_for_anal_fetish_mins(person: Person):
    person.arousal = 0
...
def abort_anal_fetish_intro(person: Person): #Use this function to exit a anal fetish scene for whatever reason (something fails, MC choice, etc.)
    person.event_triggers_dict["anal_fetish_start"] = False
...
def add_fetish_anal_staylate_event(person: Person):
    mc.business.add_mandatory_crisis(
        Fetish_Action("Employee stays late", fetish_anal_staylate_event_requirement, "fetish_anal_staylate_event_label", args = person, priority = 10, fetish_type = "anal")
    )
