...
label breeding_fetish_employee_intro_label(the_person):
    ...
    $ clear_scene()
    "[the_person.possessive_title!c] now has a fetish to get bred by you! But for today you go home."
    $ mc.change_location(bedroom)
    call advance_time(no_events = True) from _call_advance_time_breeding_fetish_employee_intro
    return True
...
label breeding_fetish_family_intro_label(the_person):
    ...
    menu:
        "Attempt to train her breeding fetish" if mc.energy > 50:
            pass
        "Attempt to train her breeding fetish\n{menu_red}Requires more energy{/menu_red} (disabled)" if mc.energy <= 50:
            pass
    ...
    mc.name "Tell you what, let's go a little further, and if it feels wrong we can stop."
    $ the_person.add_situational_slut("situation", 10, "I can stop if I want to...")
...
label breeding_fetish_generic_intro_label(the_person): #This function to be used for generic non employee, non unique girls
    ...
    if the_person.is_unique:
        "Warning, this character is unique, and may have unique fetish dialogue. If you continue, you may miss this dialogue!"
    menu:
        "Go to her place" if mc.energy > 50:
...
label breeding_fetish_lily_intro_label(the_person): #Evening room entry event
    $ the_person = lily
    $ mc.change_location(the_person.home)
    ...
    menu:
        "Hell yeah!" if mc.energy > 60:
            pass
        "Hell yeah!\n{menu_red}Requires more energy{/menu_red} (disabled)" if mc.energy <= 60:
            pass
        "Not tonight":
    ...
    the_person "Well... they've been asking to see us take things one step further... For you to finish inside me!"
    if the_person.knows_pregnant:
...
label breeding_fetish_erica_intro_label():
    ...
    if the_person.has_creampie_cum:
        ...
        $ mc.business.add_mandatory_morning_crisis(
            Fetish_Action("Erica surprises you", breeding_fetish_erica_unsuccessful_followup_requirement, "breeding_fetish_erica_unsuccessful_followup_label", fetish_type = "breeding")
        )
        return
    $ add_breeding_fetish(the_person)
...
label breeding_fetish_camila_intro_label():
    return False
