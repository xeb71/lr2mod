...
label grope_shoulder(the_person):
    ...
    if the_person.effective_sluttiness("touching_body") < 5:
        ...
        if the_person.effective_sluttiness("touching_body") < 10: #This branch is both a warning to the player not to push things too far and the way they increase sluttiness.
            ...
            "[the_person.possessive_title!c] seems uncomfortable but doesn't leave immediately."
            $ the_person.change_stats(love = -1, slut = 1, max_slut = 10)
        else:
            "[the_person.possessive_title!c] doesn't seem to mind at all as you start to gently massage her shoulder."

        menu:
            "Move your hand to her waist\n{menu_red}Costs: {energy=5}{/menu_red}" if the_person.energy >=5:
                return True

            "Move your hand to her waist\n{menu_red}Costs: {energy=5}{/menu_red} (disabled)" if the_person.energy < 5:
                pass
...
label grope_waist(the_person):
    ...
    # in future refactor to a person based dialogue grope_body_continue
    # where we can have them stop the grope chain
    if the_person == camila and not camila.event_triggers_dict.get("booty_call", False):
        the_person "Not like that [the_person.mc_title], I'm not ready for this."
        mc.name "Of course, I apologize."
        $ the_person.change_stats(happiness = -5, love = -1)
        return False
    ...
    if the_person.effective_sluttiness("touching_body") < 10:
        ...
        if the_person.effective_sluttiness("touching_body") < 15:
            ...
            $ the_person.change_stats(love = -1, slut = 1, max_slut = 15)
            # Uncomfortable
        ...
        if (the_person.vagina_available or the_person.outfit.can_half_off_to_vagina()) and (not the_person.has_taboo("vaginal_sex") or not the_person.has_taboo("anal_sex")):
            $ strip_list = the_person.outfit.get_half_off_to_vagina_list()
            if strip_list:
                python:
                    for cloth in strip_list:
                        the_person.draw_animated_removal(cloth, half_off_instead = True)
                    the_person.update_outfit_taboos()

                $ the_clothing = format_list_of_clothing(strip_list)
                "[the_person.possessive_title!c] pulls her [the_clothing] out of the way and spreads her legs."
        ...
        menu:
            "Skip the foreplay":
                call mc_move_to_private_location(the_person) from _call_mc_move_to_private_grope_waist
                call fuck_person(the_person, private = mc.location.person_count <= 1) from _call_fuck_person
                $ the_person.call_dialogue("sex_review", the_report = _return)
                call mc_restore_original_location(the_person) from _call_mc_restore_original_location_grope_waist
                return False
...
label grope_ass(the_person):
    ...
    if the_person.effective_sluttiness("touching_body") < 15:
        ...
        if the_person.effective_sluttiness("touching_body") < 20:
            ...
            $ the_person.change_stats(arousal = 1 + mc.foreplay_sex_skill, love = -1, slut = 1, max_slut = 20)
    ...
    if the_person.effective_sluttiness("touching_body") < 20:
        ...
        if the_person.effective_sluttiness("touching_body") < 25:
            ...
            if the_person.tits_visible:
                ...
                "She trails off awkwardly. You feel her nipple harden as you touch her, hidden somewhere beneath her [the_person.outfit.get_upper_top_layer.display_name]."
            $ the_person.change_stats(arousal = 3 + mc.foreplay_sex_skill, slut = 1, max_slut = 25)
            ...
            if the_person.has_taboo("touching_body"):
                ...
                $ the_person.change_arousal(3 + mc.foreplay_sex_skill)
            ...
            menu:
                "Take it further":
                    ...
                    if mc.location.person_count > 1 and the_person.opinion.public_sex < 1:
                        if the_person.arousal_perc > 30 - (the_person.opinion.public_sex * 10):
                            # dialogue is handled by caller
                            return True
                        else:
                            $ the_other_person = next((x for x in mc.location.people if x != the_person), None)
                            if the_other_person:
                                if mc.location.person_count == 2:
                                    the_person "Please, [the_person.mc_title] stop it, [the_other_person.fname] will notice us."
                                else:
                                    the_person "Please, [the_person.mc_title] stop it, [the_other_person.fname] or someone else might notice us."

                            "She seems to uncomfortable with you touching her in front of other people or maybe she's just not excited enough to continue."
                            "[the_person.possessive_title!c] seems to be on the edge of what she would tolerate."
                            "You satisfy yourself with grabbing her ass and massaging her tits while you talk."
                            return False # She is not aroused enough to take it further.
...
label quick_fuck(the_person):
    #just grab them and fuck them if they are really slutty and you have broken taboos already
    return
