label person_introduction(the_person, girl_introduction = True):
    if girl_introduction:
        if the_person.is_at_job(prostitute_job):
            ...
            # special case for professor (she will use her Prof. title on intro)
            if mc.is_at(university_hub) and the_person.has_job(university_professor_job):
    ...
    # special case for talking with a prostitute outside of work
    if the_person.has_job(prostitute_job) and not the_person.is_job_known:
        the_person "If you ever want some company, give me a call, I'm sure we can come to some kind of arrangement."
        "She hands you a business card with her phone number."
        $ mc.phone.register_number(the_person)
        $ the_person.learn_job(prostitute_job)
    return
    ...
    if ran_num > 120: #She just asks you for something "fresh". Her obedience is high enough that we already have control over this.
        ...
        menu:
            "Change what you call her":
                ...
                if title_choice == "custom":
                    $ title_choice = renpy.input("How do you want to call her?", default = the_person.name, exclude="[]{}").strip()
        ...
        if the_person.title == the_person.create_formatted_title(title_one) or the_person.title == the_person.create_formatted_title(title_two):  #If we picked the one we're currently using we have a slightly different dialogue setup.
            ...
            menu:
                ...
                "You will call her...":
                    $ title_choice = renpy.input("How do you want to call her?", default = title_one, exclude="[]{}").strip()
                    if len(title_choice) == 0:
                        $ title_choice = title_two
                    mc.name "[title_choice] does have a nice ring to it. You should start using that."
                    $ the_person.set_title(title_choice)
                    the_person "I think you're right. Thanks for the input!"
    ...
    if ran_num > 120: #She just asks you for something "fresh". Her obedience is high enough that we already have control over this.
        ...
        menu:
            "Change what she calls you":
                ...
                if title_choice == "custom":
                    $ title_choice = renpy.input("What should she call you?", default = mc.name, exclude="[]{}").strip()
        ...
        if the_person.mc_title == title_one or the_person.mc_title == title_two:  #If we picked the one we're currently using we have a slightly different dialogue setup.
            ...
            menu:
                ...
                "She should call you...":
                    $ title_choice = renpy.input("What should she call you?", default = title_one, exclude="[]{}").strip()
                    if len(title_choice) == 0:
                        $ title_choice = title_two
                    mc.name "[title_choice] does have a nice ring to it. You should start using that."
                    $ the_person.set_mc_title(title_choice)
...
label small_talk_person(the_person, apply_energy_cost = True, is_phone = False): #Tier 0. Useful for discovering a character's opinions and the first step to building up love.
    ...
    if not is_phone and the_person.love > 5 and the_person.primary_job and not the_person.primary_job.job_known:
        ...
        $ the_person.primary_job.job_known = True
    elif not is_phone and the_person.love > 20 and not the_person.is_job_known:
        $ ran_num = renpy.random.randint(0, 1)
        ...
        if not opinion_learned is None:
            ...
            $ the_person.change_stats(happiness = the_person.opinion.small_talk + 1,
                love = 3 - prediction_difference, max_love = 25,
                obedience = mc_serum_aura_obedience.trait_tier if not is_phone and mc_serum_aura_obedience.is_active else 0,
                arousal = mc_serum_aura_arousal.trait_tier if not is_phone and mc_serum_aura_arousal.is_active else 0)
        ...
        if the_person.love > 20 and the_person.has_role(instapic_role) and not the_person.instapic_known:
...
label compliment_person(the_person): #Tier 1. Raises the character's love. #TODO: just have it raise love and not sluttiness.
    ...
    $ the_person.change_stats(happiness = the_person.opinion.flirting,
        love = int(min(4, max(mc.charisma / 2.0, 1))), max_love = 20,
        obedience = mc_serum_aura_obedience.trait_tier if mc_serum_aura_obedience.is_active else 0,
        arousal = mc_serum_aura_arousal.trait_tier if mc_serum_aura_arousal.is_active else 0)
    if the_person.opinion.flirting > 0:
        the_person "It's been fun talking [the_person.mc_title], we should do this again sometime!"
    else:
        the_person "[the_person.mc_title], if that's all you had to say, I'd like to go."
    $ the_person.apply_serum_study()
...
label flirt_person(the_person): #Tier 1. Raises a character's sluttiness up to a low cap while also raising their love by less than a compliment.
    ...
    if the_person.is_girlfriend:
        ...
        if the_person.outfit.outfit_slut_score > 50:
            if the_person.vagina_visible:
                ...
                mc.name "[the_person.title], you have wonderful breasts, any chance you would let me play with them?"
            elif the_person.shows_off_her_tits:
    ...
    python:
        ...
        the_person.change_stats(happiness = the_person.opinion.flirting,
            arousal = change_amount + (mc_serum_aura_arousal.trait_tier if mc_serum_aura_arousal.is_active else 0),
...
label lunch_date_plan_label(the_person):
    if get_named_label("lunch_date_plan_label", the_person):
        $ run_named_label("lunch_date_plan_label", the_person)
        return
    if the_person == kaya and the_person.is_at_job("Barista"):
    ...
    call date_schedule_selection(the_person, 2) from _date_schedule_afternoon_lunch_in_person_01
    if _return:
...
label movie_date_plan_label(the_person):
    if get_named_label("movie_date_plan_label", the_person):
        $ run_named_label("movie_date_plan_label", the_person)
        return
    ...
    if the_person.has_role(sister_role):
        ...
        if time_of_day <= 3:
            ...
            the_person "Oh, a movie sounds fun! I don't have anything going on tomorrow night, would that work for you?"

    call date_schedule_selection(the_person, 3) from _date_schedule_evening_movie_in_person_01
    if _return:
...
label dinner_date_plan_label(the_person):
    if get_named_label("dinner_date_plan_label", the_person):
        $ run_named_label("dinner_date_plan_label", the_person)
        return
    ...
    call date_schedule_selection(the_person, 3) from _date_schedule_evening_dinner_in_person_01
    if _return:
...
label bar_date_plan_label(the_person):
    if get_named_label("bar_date_plan_label", the_person):
        $ run_named_label("bar_date_plan_label", the_person)
        return
    mc.name "Hey, I was thinking about hitting up the bar and getting a few drinks tonight. You want to join me?"
    the_person "Oh, I suppose I would be up for that. When and where?"
    call date_schedule_selection(the_person, 4) from _date_schedule_night_bar_in_person_01
    if _return:
...
label bc_talk_label(the_person):
    ...
    # First check and see if we have a unique override.
    if get_named_label("bc_talk_label", the_person):
        $ run_named_label("bc_talk_label", the_person)
        return
            ...
            "Stop taking birth control" if the_person.on_birth_control and the_person.has_event_delay("changed_bc", 7):
                ...
                if the_person.event_triggers_dict.get("mandate_bc", False):
                    ...
                    if the_person == mom:
                        ...
                        the_person "I'm sorry, but I have personal reason for being on birth control. Please don't ask me to do that."
                elif the_person.love >= needed_stop or the_person.effective_sluttiness() >= needed_stop:
                    if the_person.wants_creampie:
...
label bc_demand_label(the_person):
    ...
    # First check and see if we have a unique override.
    if get_named_label("bc_demand_label", the_person):
        $ run_named_label("bc_demand_label", the_person)
        return
...
label use_energy_perk_in_conversation_label(the_person):
    python:
