label strip_awkward_stand_intro(the_person, guy_state, for_pay = False):
    "[the_person.possessive_title!c] stands in front of you, doing her best to avoid making eye contact."
...
label strip_awkward_transition(the_person, guy_state, for_pay = False):
    "[the_person.title] stands a few {long_height_system} in front of you. She seems uncomfortable with all the attention and doesn't know what to do."
...
label strip_awkward_stand_towards_2(the_person, guy_state, for_pay = False):
    if the_person.tits_visible:
        "[the_person.possessive_title!c] wiggles her shoulders half-heartedly."
        if the_person.has_large_tits:
            "Luckily, her big tits make up for her lack of enthusiasm, jiggling around with every movement."
    else:
        "[the_person.possessive_title!c] wiggles her shoulders half-heartedly."
        if the_person.has_large_tits:
            "Unfortunately, her [the_person.outfit.get_upper_top_layer.display_name] is able to keep her tits from doing much jiggling."
        else:
            "Unfortunately there isn't much for her to shake. Any interesting movement is hidden underneath her [the_person.outfit.get_upper_top_layer.display_name]."
...
label strip_awkward_stand_away_1(the_person, guy_state, for_pay = False):
    ...
    if the_person.vagina_visible:
        ...
        "She runs a hand over her [the_person.outfit.get_lower_top_layer.display_name], then gives herself the world's lightest spank."
    mc.name "Looking good [the_person.title]."

    if guy_state == "jerking":
        "You enjoy the view and stroke your cock some more."
        "[the_person.possessive_title!c] stares at your cock for a moment, then pulls her eyes away."
    return

label strip_awkward_stand_climax(the_person, guy_state, for_pay = False): #TODO: Link this with some personality dialogue
    "[the_person.possessive_title!c] looks away from you, panting loudly."
...
label strip_awkward_stand_outro(the_person, guy_state, for_pay = False): #TODO: Decide if we want to let you resist/force cumming
    ...
    if guy_state == "watching" or guy_state == "touching":
        ...
        "You grunt and shut your eyes as your cock pumps its load into your underwear."
...
#     $ generalised_strip_description(the_person, the_clothing)
    ...
    #
    #     #TODO: Remind hr that you're paying while she strips
    #
    # else:
