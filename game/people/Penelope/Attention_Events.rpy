label attention_event():
    ...
    if not city_rep.is_available:
        return # she's not available, so the city won't sent her over
    ...
    $ city_rep.change_location(lobby)
    ...
    if bribe_result == "cash":
        ...
        call sex_review_trance(city_rep) from _call_sex_review_trance_attention_event
    ...
    $ city_rep.draw_person(position = "walking_away")
    "She leaves the building with her city thugs following behind her."

    $ city_rep.change_location(city_hall)
    $ mc.business.attention = builtins.int(mc.business.attention/2)
    $ city_rep.event_triggers_dict["attention_times_visited"] = city_rep.event_triggers_dict.get("attention_times_visited", 0) + 1
    ...
    $ mc.stats.change_tracked_stat("Business", "City Rep. Visits", 1)
    $ clear_bribe_attempts() #Reset our list so we won't accidentally trigger something outside of this event.
    $ clear_scene()
    "You're glad to finally be left alone, but the encounter has eaten up most of the day."
    return "Advance Time"

label attention_pay_fine(the_person):
    $ fine_amount = builtins.round(sum(x.primary_job.salary for x in mc.business.employee_list) * 0.01) * 1000 #ie. costs 10x a days staff cost, rounded to the nearest $100
    if fine_amount < 100:
        $ fine_amount = 100
    ...
    $ mc.business.change_funds(-fine_amount, stat = "City Fines")
    $ mc.log_event(f"$ {fine_amount:.0f} seized!", "float_text_red")
    return

label attention_seize_inventory(the_person):
    $ highest_attention_design = get_highest_attention_serum_design_from_inventory()

    if highest_attention_design is None:
        "[the_person.title]'s enforcers return."
        "Enforcer" "Apologies Ma'am, we couldn't find anything."
        the_person "It seems you are in luck, [the_person.mc_title]."
        mc.name "Like I told you [the_person.title], I have nothing to hide."
        return

    $ doses_seized = seize_doses_from_inventory(highest_attention_design)
    ...
    "[the_person.possessive_title!c] flips open one of the boxes and pulls out the plastic vial inside."
    the_person "Hmmm, [highest_attention_design.name]. How... descriptive."
    "She slips it back into its box and turns back to you."
    if the_person.event_triggers_dict.get("city_rep_reduced_penalties_trained", False):
    ...
    $ mc.log_event(f"{doses_seized} doses of {highest_attention_design.name} seized!", "float_text_red")
    $ mc.stats.change_tracked_stat("Business", "Serums Doses Seized", doses_seized)
...
label attention_seize_supplies(the_person):
    ...
    $ mc.log_event(f"{'All' if mc.business.supply_count == 0 else supply_seized} serum supply seized!", "float_text_red")
    $ mc.stats.change_tracked_stat("Business", "Supplies Seized", supply_seized)
    return
...
label attention_seize_research(the_person):
    ...
    "[the_person.possessive_title!c] snatches the top file and flips through it."
    ...
    if the_person.event_triggers_dict.get("city_rep_reduced_penalties_trained", False):
        ...
        "Enforcer" "Are you sure? all the reports say..."
        ...
        $ mc.log_event(f"{highest_attention_design.name} design seized!", "float_text_red")

    $ mc.stats.change_tracked_stat("Business", "Serums Designs Seized", 1)
    $ highest_attention_design = None
    return
...
label attention_illegal_serum(the_person):
    ...
    "[the_person.possessive_title!c] takes one of the vial and looks it over."
    ...
    $ highest_attention_design.change_attention(highest_attention_design_change)
    $ mc.log_event(f"{highest_attention_design.name} made illegal: +{highest_attention_design_change} Attention!", "float_text_red")
    $ mc.stats.change_tracked_stat("Business", "Serums Penalised", 1)
    $ highest_attention_design = None
    return

label attention_already_in(the_person):
    $ times_visited = the_person.event_triggers_dict.get("attention_times_visited", 0)

    if times_visited == 0:
        $ city_rep_first_visit_unlock(the_person)
...
label attention_call_to_work(the_person):
    ...
    if times_visited == 0:
        ...
        $ play_ring_sound()
        "Your phone rings, a call from an unknown number."
        mc.name "Hello?"
        ...
        $ play_ring_sound()
        "Your phone rings. A call from [the_person.title]."
...
label attention_coffee(the_person):
    ...
    $ ran_num = renpy.random.randint(0, 1)  # 50% chance she will agree, otherwise based on love
    if not ran_num != 1 and the_person.love < 0:
        "[the_person.possessive_title!c] gives you an icy glare."
        the_person "No. Thank you."

    elif not ran_num != 1 and the_person.love < 15:
        the_person "I'm not supposed to accept anything from the subject of an investigation, but..."
        "[the_person.possessive_title!c] considers it for a moment, then gives you a polite nod."
        ...
        the_person "Some coffee sounds lovely, [the_person.coffee_style], please. Thank you [the_person.mc_title]."
        mc.name "I'll be back in a second and..."
        the_person "Oh, I should really come with you. I'm supposed to keep you under observation at all times."
        ...
        menu:
            "Add a dose of serum to her coffee" if mc.inventory.has_serum:
            ...
            "Add a dose of serum to her coffee\n{menu_red}Requires: Serum in Inventory{/menu_red} (disabled)" if not mc.inventory.has_serum:
                pass
        ...
        $ mc.change_location(lobby)
        $ the_person.draw_person()
        "You return to the lobby, coffee in hand. [the_person.possessive_title!c] gives a happy sigh as you hand her a cup."
        the_person "Ah... I think I would go insane without coffee. Some days it's the only breakfast I get."
