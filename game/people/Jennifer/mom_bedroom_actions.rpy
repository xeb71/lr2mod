init -1 python:
    def get_magazine_name(person, discover_opinion = False): # Returns the name of a porn magazine this person might be interested in reading, based on her opinions.
        if person.opinion.drinking_cum > 0:
            if discover_opinion:
                person.discover_opinion("drinking cum")
            return "Cum Guzzlers Monthly"
        elif person.opinion.creampies > 0:
            if discover_opinion:
                person.discover_opinion("creampies")

            if person.opinion.bareback_sex > 0:
                if discover_opinion:
                    person.discover_opinion("bareback sex")
                return "Stroked - Sluts Fucked Pregnant!"
            else:
                return "Close Up Cum Shots - Filled to the Brim!"
        elif person.opinion.being_covered_in_cum > 0:
            if discover_opinion:
        ...
        elif not person.has_family_taboo:
            if discover_opinion:
                person.discover_opinion("incest")
            ...
            return "Public Sluts"
        elif person.opinion.lingerie > 0:
            if discover_opinion:
                person.discover_opinion("lingerie")
            return "Lingerie Monthly: What to Wear"
        elif person.opinion.giving_handjobs > 0:
            if discover_opinion:
                person.discover_opinion("giving handjobs")
            return "Getting Handy with Handjobs."
        elif person.opinion.giving_tit_fucks > 0:
            if discover_opinion:
                person.discover_opinion("giving tit fucks")
            return "Bimbo Quarterly - Titfuck Edition"
        elif person.is_submissive:
            if discover_opinion:
                person.discover_opinion("being submissive")
    ...
    menu:
                ...
                if the_person.event_triggers_dict.get("BC_nightstand_hide_day",-7) < day:
                    menu:
                        "Hide her birth control":
                            $ the_person.event_triggers_dict["BC_nightstand_hide_day"] = day #Can't re-hide it today, because she never stops until tomorrow anyways.
        ...
        "Check her computer":
            ...
            if the_person.event_triggers_dict.get("known_computer_password", False): #If you don't know the password yet, try and guess it.
                ...
                while not success:
                    menu:
                        "Try again":
                            "COMPUTER" "INPUT PASSWORD"
                            $ password_attempt = renpy.input("HINT: The oldest is called")
                            if password_attempt.lower() == mc.name.lower():
                                $ the_person.event_triggers_dict["known_computer_password"] = True
                                $ success = True
                            else:
            ...
            if the_person.event_triggers_dict.get("known_computer_password", False): #If you know the password at this point, no problem logging in."
                "COMPUTER" "WELCOME [the_person.title]!"
                ...
                if the_person.event_triggers_dict.get("mom_vaginal_quest_active", False) and the_person.event_triggers_dict.get("mom_vaginal_quest_progress", 0) == 0:
                    ...
                    "COMPUTER" "RESTORE BROWSING SESSION?"
                    ...
                    "Anon3342" "{b}(Advice Wanted) My Sex Drive is Back!?{/b}"
                    "Anon3342" "I'm the lucky single mother of two wonderful children. Both are growing up so fast, and are starting to leave the nest"
                    "Anon3342" "Raising them has always been my top priority, but lately something feels different. My libido has sky-rocketed!"
                    "Anon3342" "I thought I was going to be in mother-mode for the rest of my life, but I feel like I'm a horny teenager again!"
                    "Anon3342" "Now I don't know what to do! Should I ignore this to make sure I'm around for my children 100%%?"
                    "Anon3342" "Looking for advice, signed: a horny Mom!"
                    "Did [the_person.title] write this? You scroll down to see the responses."
                    "Anon1449" "Don't feel guilty Horny Mom, you've spent your whole life caring for them. Now it's time to enjoy yourself!"
                    "MTeresa" "You aren't going to be young forever Horny Mom. Get out and meet a new man while you still can."
                    "Jocasta1" "There's at least one way to satisfy your needs without ignoring your family. Do you have a son?"
                    "Anon3342" "I do have a son Jocasta1. I also have a daughter."
                    "Jocasta1" "Well then let him take care of your sex drive! He's probably filled with hormones, and you'll be so much closer!"
        ...
        "Look in her dresser":
            ...
            # TODO: Decide if we want to have extra options here for peeking at wardrobe stuff.
