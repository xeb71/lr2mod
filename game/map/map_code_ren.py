...
from game.major_game_classes.game_logic.Room_ren import Room, list_of_places, sex_store
...
def gym_is_open():
    return sports_center_is_open()

def sports_center_is_open():
    return time_of_day != 4

def sex_shop_is_open():
    return sex_store.visible and time_of_day in (1, 2, 3) and day % 7 != 6    # unlocked and not closed on sundays
...
def build_tile_information(known_people: list[Person], total_people: int, location_name: str, has_event: bool, has_progress: bool) -> str:
    ...
    if any(x for x in known_people if x.is_queen):
        extra_info.append("{image=full_crown_token_small}")
    ...
    if len(extra_info) > 6:
        ...
        #print(f"Insert tile break at {break_insert}")
