...
from game.text_tags.kinetic_text_tags_ren import DispTextStyle
...
class ExplodeText(renpy.Displayable):
    def __init__(self, child, timer=2, **kwargs) -> None:
...
class ExplodeHalfText(renpy.Displayable):
    def __init__(self, child, length, id, explode_point, timer=2, **kwargs) -> None:
        super().__init__(**kwargs)
        self.child = child
        self.curr_x = 0
        self.curr_y = 0
...
# center: (int) Position in the string the explosion will be centred on.
# timer: (float) How long till the text explodes
...
def explodehalf_tag(tag, argument, contents):
    ...
    for kind, text in contents:
        ...
        elif kind == renpy.TEXT_TAG and text.find("image") != -1:
            curr_id += 1
    curr_id = 0
    if center_arg == -1:
